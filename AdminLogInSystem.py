...
db = mysql.connector.connect(host="localhost",user="root",passwd="root",database="admin")
mycur = db.cursor()
...
def register_user():
    ...
    if username_info == "":
        ...
        sql = "insert into administartors values(%s,%s)"
...
def logged():
    ...
    Button(logg, text="Continue", bg="grey", width=8, height=1, command=logg_destroy).pack()
...
def login_varify():
    uv = username_varify.get()
    pv = password_varify.get()
    sql = "select * from administartors where Name = %s and Password = %s"
    mycur.execute(sql,[(uv),(pv)])
...
def main_screen():
    ...
    Button(root, text="Create Account", command=ada).place(x=10, y=100)
